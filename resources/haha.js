...
Settings_Vtiger_Edit_Js("Settings_Workflows_Edit_Js", {
   ...
   registerEventForShowModuleFilterCondition: function () {
    ...
    registerOwnerFieldEvent: function (container) {
        ...
        var assignParentRecordOwnersInput = container.find('input[name="assign_parent_record_owners"]');
        ...
        function displayOnwerFieldStatus() {
            if (assignParentRecordOwnersInput.is(':checked')) {
                ...
                assignedUsersInput.closest('.fieldValue').find('.input-error').removeClass('input-error');
            }
            else {
   ...
   registerEditTaskEvent: function () {
      ...
      container.on('click', '[data-url]', function (e) {
         ...
         .then(function (error, data) {
            ...
            .then(function (container) {
               thisInstance.initTaskFormModal(container);
            });
         });
      });

      // Edit existing workflow task
      container.on('click', '.editTask', function (e) {
         ...
         .then(function (error, data) {
            ...
            app.helper.loadPageContentOverlay(data)
            .then(function (container) {
               thisInstance.initTaskFormModal(container, taskData);
            });  
         });
      });
   ...
   // Extracted this function here by Hieu Nguyen on 2021-11-16 to use in common for both new and edit cases
   initTaskFormModal: function (container, taskData) {
      ...
      var taskType = container.find('#taskType').val();
      ...
      // Added by Hieu Nguyen on 2021-11-16 to set validation rules for SMS content field
      if (taskType == 'VTSMSTask') {
         let contentInput = container.find('[name="content"]');
      ...
      // Added by Hieu Nguyen on 2020-10-26 to render custom owner field after Workflow Task Edit popup display
      CustomOwnerField.initCustomOwnerFields(container.find('input[name="assigned_user_id"]'));
      thisInstance.registerOwnerFieldEvent(container);
      // End Hieu Nguyen

      // Added by Hieu Nguyen on 2020-10-26 to support assign new record to parent record owners
      if (taskData) {
         taskData = JSON.parse(taskData); // Temporary data here

         if (taskData && taskData.assign_parent_record_owners == 'on') {
            container.find('[name="assign_parent_record_owners"]').attr('checked', true);
         }
      }
      // End Hieu Nguyen
   },
   ...
   getValues: function (tasktype) {
      ...
      conditions.each(function (i, conditionDomElement) {
         ...
         var rowValues = {};
         if (fieldType == 'owner') {
            // Modified by Hieu Nguyen on 2020-07-02 to collect value for custom owner field
			...
			// Modified by Hieu Nguyen on 2021-11-22 to save selected value for picklist and multipicklist field
            for (var key in fieldList) {
               	var field = fieldList[key];
   ...
   loadFieldSpecificUi: function (fieldSelect) {
      ...
      if (fieldSpecificUi.is('input.select2')) {
        ...
        if (fieldModel.getName() == 'assigned_user_id' || fieldModel.getName() == 'main_owner_id') {
            ...
            var selectedTags = (fieldModel.getValue()) ? JSON.stringify(fieldModel.getValue()) : '';
            fieldSpecificUi = '<input type="text" name="'+ fieldModel.getName() +'" value="" data-selected-tags=\''+ selectedTags +'\' data-value="value" data-rule-main-owner="true" class="form-control">' +
                '<input type="hidden" name="valuetype" value="rawtext" />';
                
            // Added by Hieu Nguyen on 2020-10-26 to support assign new record to parent record owners
            if (fieldModel.getName() == 'assigned_user_id' && jQuery.inArray(taskType, ['VTCreateTodoTask', 'VTCreateEventTask', 'VTCreateEntityTask']) >= 0) {
                var checked = fieldUiHolder.data('assignParentRecordOwners') == 1 ? 'checked' : '';
                fieldSpecificUi += '<label><input type="checkbox" name="assign_parent_record_owners" '+ checked +' /> '+ fieldUiHolder.data('assignParentRecordOwnersLabel') +'<label>';
            }
            // End Hieu Nguyen
        ...
        if (fieldModel.getName() == 'createdby' || fieldModel.getName() == 'modifiedby' || fieldInfo.column == 'inventorymanager') {
            ...
            fieldSpecificUi = '<input type="text" name="'+ fieldModel.getName() +'" value="'+ fieldModel.getValue() +'" data-selected-tags="'+ selectedTags +'" data-value="value" data-rule-main-owner="true" class="form-control">' +
                '<input type="hidden" name="valuetype" value="rawtext" />';
            fieldUiHolder.html(fieldSpecificUi);
   ...
   // Register event for show module filter condition
   registerMassActionBlock: function () {
      var thisInstance = this;
      var editViewContainer = this.getEditViewContainer();

      $('.bootstrap-switch').bootstrapSwitch();

      editViewContainer.on('click', 'input[name="workflow_trigger"]', function (e) {
         var currentTarget = $(e.currentTarget);
         var massActionContainer = editViewContainer.find('.workflowMassActionBlock');
         
         if (['1', '2', '3'].includes(currentTarget.val())) {
            massActionContainer.toggleClass('hide', false);
         }
         else {
            massActionContainer.toggleClass('hide', true);
         }
      });

      // Trigger click event on load
      editViewContainer.find('input[name="workflow_trigger"]:checked').trigger('click');
   },
