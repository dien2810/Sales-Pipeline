...
class Settings_PipelineConfig_EditPipelineAjax_View extends CustomView_Base_View {
	...
	function getSendEmailModal(Vtiger_Request $request) {
		...
		// Lấy danh sách các trường email
		$emailFieldOptions = '';
		...
		$fromEmailFieldOptions .= '<option value="$(general : (__VtigerMeta__) supportName)<$(general : (__VtigerMeta__) supportEmailId)>">'
								. vtranslate('LBL_HELPDESK_SUPPORT_EMAILID', $moduleName) . '</option>';
		...
		// Lấy danh sách tất cả field options
		$allFieldOptions = '';
		...
		// Lấy thông tin Task Object từ request
		$taskObject = $request->get('taskObject');
